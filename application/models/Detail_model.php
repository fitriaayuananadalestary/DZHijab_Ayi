...
class detail_model extends CI_Model
{
    ...
    public function get()
    {
        ...
        $this->db->join('hijab s', 'k.id_buku=s.id');
        $query = $this->db->get();
        return $query->result_array();
    }
    public function getById($id)
    {
        $this->db->select('d.*,r.nama as nama, s.nama as hijab');
        $this->db->from('detail_penjualan d');
        $this->db->join('user r', 'd.id_user = r.id');
        $this->db->join('hijab s', 'd.id_hijab = s.id');
    ...
    public function getByUser($id)
    {
        ...
        $this->db->select('d.*, s.nama as nama_hijab');
        $this->db->from('detail_penjualan d');
        $this->db->join('hijab s', 'd.id_hijab = s.id');
    ...
    function charts()
    {
        $this->db->select('d.*, s.nama as hijab, sum(d.jumlah) as jum');
        $this->db->from('detail_penjualan d');
        $this->db->join('hijab s', 'd.id_hijab = s.id');
